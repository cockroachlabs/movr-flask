...
def end_ride_txn(session, city, ride_id, location):
    ...
    """
    r = session.query(Ride).filter(
        Ride.city == city, Ride.id == ride_id).first()
    v = session.query(Vehicle).filter(Vehicle.city == city,
...
def add_user_txn(session, city, first_name, last_name, email, username, password, password_hash=None, is_owner=False):
    ...
    """
    u = User(city=city, id=str(uuid.uuid4()), first_name=first_name,
             last_name=last_name, email=email, username=username, is_owner=is_owner)
...
def add_vehicle_txn(session, city, owner_id, last_location, type, color, brand, status, is_owner):
    ...
    """
    vehicle_type = type
    v = Vehicle(id=str(uuid.uuid4()), type=vehicle_type, city=city, owner_id=owner_id,
                last_location=last_location, color=color, brand=brand, status=status)
    session.add(v)
    if not is_owner:
        u = session.query(User).filter(User.city == city,
...
def remove_vehicle_txn(session, city, id):
    ...
    """
    v = session.query(Vehicle).filter(
        Vehicle.city == city, Vehicle.id == id).first()
    v.status = 'removed'
    owner = v.owner_id
    vehicles = session.query(Vehicle).filter(
        Vehicle.city == city, Vehicle.owner_id == owner, Vehicle.status != 'removed').all()
    if not vehicles:
        u = session.query(User).filter(
            User.city == city, User.id == owner).first()
        u.is_owner = False
...
def get_users_txn(session, city):
    ...
    users = session.query(User).filter(
        User.city == city, User.username != None).all()
